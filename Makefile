...
ENV.VARS.ROOT=export JAVA_HOME=$(JAVA.HOME) && export ANDROID_HOME=$(ANDROID.SDK.DIR)/platforms && export PATH=$(ANDROID.HOME)/tools:$(ANDROID.HOME)/platform-tools:$(ANDROID.SDK.DIR)/bin:$(PWD)/node_modules/.bin:$(PATH)  && export ANDROID_SDK_ROOT=$(ANDROID.SDK.DIR)/platforms
...
ANDROID.BUNDLE.RELEASE.ORIG=$(BASE.DIR)/android/app/build/outputs/bundle/release/app-release.aab
...
APK.RELEASE.ORIG=$(BASE.DIR)/android/app/build/outputs/apk/release/app-release.apk
...
install.node: .FORCE
#	$(info $(ENV.VARS))
	$(ENV.VARS) && $(NVM.VARS) && $(MAKE) nvm.use && npm install react-native-cli --legacy-peer-deps && npm install --legacy-peer-deps
ifeq ($(OS), Darwin)
	$(ENV.VARS) && $(NVM.VARS) && $(MAKE) nvm.use && npm install ios-deploy --legacy-peer-deps
...
build.android.react.bundle: install.node
	$(ENV.VARS) && $(MAKE) nvm.use && react-native bundle --verbose --platform android --dev false --entry-file index.js --bundle-output android/app/src/main/assets/index.android.bundle --assets-dest android/app/src/main/res

dist.directory: .FORCE
	mkdir -p $(DIST.DIR)

build.android.bundle.debug: dist.directory build.android.react.bundle
	$(ENV.VARS) && cd $(ANDROID.DIR) && ./gradlew bundleDebug
	$(MAKE) deploy.android.bundle.debug

build.android.bundle.release: dist.directory build.android.react.bundle
	$(ENV.VARS) && cd $(ANDROID.DIR) && ./gradlew bundleRelease
...
build.apk.release: build.android.react.bundle
	grep -R '__DEV__=true' $(BASE.DIR)/src
	@echo $? #TODO: add check here to make sure we don't ship code with __DEV__=true
	$(ENV.VARS) && cd $(ANDROID.DIR) && ./gradlew assembleRelease
	$(MAKE) deploy.apk.release

build.apk.debug: build.android.react.bundle
	$(ENV.VARS) && cd $(ANDROID.DIR) && ./gradlew assembleDebug
...
build.ios.react.bundle: install.node
	$(ENV.VARS) && $(MAKE) nvm.use && react-native bundle --verbose --platform ios --dev false --entry-file index.js --bundle-output $(IOS.DIR)/main.jsbundle --assets-dest $(IOS.DIR)

install.pods: .FORCE
	$(ENV.VARS) && cd $(IOS.DIR) && pod repo update && pod update && pod install --verbose --repo-update
...
ifdef MACOS_KEYCHAIN_PASSWORD
	...
	$(ENV.VARS) && fastlane run setup_circle_ci && cd $(IOS.DIR) && $(XCODEBUILD.BIN)  -workspace $(IOS.WORKSPACE) -configuration Debug archive -sdk iphoneos -scheme $(APP.NAME) -archivePath $(IOS.XCARCHIVE)
...
ifdef MACOS_KEYCHAIN_PASSWORD
	...
	$(ENV.VARS) && fastlane run setup_circle_ci && cd $(IOS.DIR) && $(XCODEBUILD.BIN)  -workspace $(IOS.WORKSPACE) -configuration Release archive -sdk iphoneos -scheme $(APP.NAME) -archivePath $(IOS.XCARCHIVE)
